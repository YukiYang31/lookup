...
application { mainClass = "org.plumelib.lookup.Lookup" }
...
spotless {
  ...
  kotlinGradle {
    // The default target is every "*.gradle.kts" file in the project directory.
...
//
// This is an object rather than a function of the build script, because a
// function of the build script is a method of the script object, so a task
// action that called it would capture that object, which the configuration
...
object JavadocFonts {
  ...
  fun removeDejaVuFontImport(javadocDir: File) {
    ...
    for (file in javadocDir.walkTopDown()) {
...
// `Javadoc.options` is declared as `MinimalJavadocOptions`, which lacks the methods that add
// options.  Every Javadoc task in this build uses the standard doclet's options object.
fun standardOptions(javadocTask: Javadoc) = javadocTask.options as StandardJavadocDocletOptions

tasks.withType<Javadoc>().configureEach {
  standardOptions(this).isNoTimestamp = true
  options.quiet()
...
fun strictJavadoc(javadocTask: Javadoc) {
  standardOptions(javadocTask).addBooleanOption("Xdoclint:all", true)
  standardOptions(javadocTask).addBooleanOption("Xwerror", true)
}
...
val javadocPrivate =
  tasks.register<Javadoc>("javadocPrivate") {
    ...
    standardOptions(this).addBooleanOption("private", true)
    strictJavadoc(this)
  }
...
val requireJavadoc =
  tasks.register<JavaExec>("requireJavadoc") {
    ...
    // A local variable, because reading the script's `requireJavadocMarker` property from a task
    // action would capture the script object, which the configuration cache cannot serialize.
    val markerProvider = requireJavadocMarker
    ...
    doLast {
      val marker = markerProvider.get().asFile
...
tasks.register<Javadoc>("updateUserOptions") {
  ...
  val javadocOptions = standardOptions(this)
  javadocOptions.memberLevel = JavadocMemberLevel.PRIVATE
  javadocOptions.doclet = "org.plumelib.options.OptionsDoclet"
  javadocOptions.addStringOption(
    "docfile",
    "$projectDir/src/main/java/org/plumelib/lookup/Lookup.java",
  )
  javadocOptions.addStringOption("format", "javadoc")
  // OptionsDoclet's "-i" means "edit the docfile in place"; it takes no value.
  javadocOptions.addBooleanOption("i", true)
  // Overrides the blanket `isNoTimestamp = true` in the `withType<Javadoc>` block above.
  javadocOptions.isNoTimestamp = false
  title = ""
  ...
  // Calls neither `standardOptions` nor any other function of this script, because doing so would
  // capture the script object, which the configuration cache cannot serialize.
  doFirst { (this as Javadoc).options.docletpath = mainRuntimeClasspath.files.toList() }
}
